...
const LiveChart = ({ batchMetrics, epochMetrics, totalEpochs, isTraining }) => {
  const hasData = batchMetrics.length > 0 || epochMetrics.length > 0;
...
const TrainingConfig = ({ projectId, nodes, edges }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto w-full space-y-6">
      ...
      {/* ── Live chart — shown at top as soon as training starts ── */}
      {(training || batchMetrics.length > 0 || metrics.length > 0) && (
        <LiveChart
      ...
      {/* Start / Stop buttons */}
      <div className="flex gap-4 animate-fade-in-up pt-2">
        <button
