...
const DatasetUpload = ({ projectId }) => {
  ...
  const handleLoadBuiltin = async (key) => {
    ...
    try {
      ...
      setActiveDataset(data);
      toast.success(
        `${data.name} loaded! ${data.num_train.toLocaleString()} training samples ready.`,
      );
    } catch (err) {
  ...
  const handleUpload = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      setFiles((prev) =>
        prev.map((file, idx) =>
          idx === i ? { ...file, status: "uploading", progress: 30 } : file,
        ),
      );
      ...
      try {
        ...
        setFiles((prev) =>
          prev.map((file, idx) =>
            idx === i ? { ...file, status: "done", progress: 100 } : file,
          ),
        );
      } catch (err) {
        setFiles((prev) =>
          prev.map((file, idx) =>
            idx === i ? { ...file, status: "error", progress: 0 } : file,
          ),
        );
    ...
    <div className="max-w-4xl mx-auto w-full space-y-6">
        ...
        <div className="flex items-end gap-4">
          <div className="flex-1">
            ...
            <p className="text-white text-lg leading-relaxed max-w-xl">
              Select a built-in Keras dataset for rapid prototyping, or upload
              your own custom data in CSV or image formats.
            </p>
          </div>
          <div className="px-4 py-2 rounded-lg bg-cyan/20 border border-cyan/50 font-mono text-xs text-cyan font-bold tracking-wider">
            SOURCE MANAGEMENT
          </div>
        </div>
      </div>
      ...
      {activeDataset && activeDataset.active !== false && (
        <div className="p-4 rounded-2xl bg-cyan/10 border border-cyan/30 flex items-center justify-between animate-fade-in-up">
          <div className="flex items-center gap-4">
            ...
            <div>
              <strong className="block text-white font-bold mb-1">
                {activeDataset.name || activeDataset.dataset}
              </strong>
              <span className="text-xs text-dim font-mono flex gap-2">
                {activeDataset.input_shape && (
                  <span className="px-2 py-0.5 rounded border border-white/10 bg-white/5">
                    Shape: {activeDataset.input_shape}
                  </span>
                )}
                {activeDataset.num_classes && (
                  <span className="px-2 py-0.5 rounded border border-white/10 bg-white/5">
                    {activeDataset.num_classes} classes
                  </span>
                )}
                {activeDataset.num_train && (
                  <span className="px-2 py-0.5 rounded border border-white/10 bg-white/5">
          ...
          <span className="px-3 py-1 rounded-lg border border-cyan/40 text-cyan bg-cyan/10 font-mono text-xs uppercase tracking-wider font-bold">
            Active
          </span>
        </div>
      )}
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  ...
                  {isActive && (
                    <span className="px-2 py-1 rounded bg-cyan/20 text-cyan text-xs font-mono font-bold uppercase">
                      Active
                    </span>
                  )}
                </div>
                ...
                <p className="text-sm text-dim leading-relaxed mb-4 flex-1">
                  {ds.description}
                </p>

                <div className="flex flex-wrap gap-2 mb-6">
                  <span className="px-2 py-1 rounded border border-white/5 bg-[#0B0B0F] text-dim font-mono text-[10px]">
                    Shape: <span className="text-white">{ds.input_shape}</span>
                  </span>
                  <span className="px-2 py-1 rounded border border-white/5 bg-[#0B0B0F] text-dim font-mono text-[10px]">
                    <span className="text-white">{ds.num_classes}</span> classes
                  </span>
                  <span className="px-2 py-1 rounded border border-white/5 bg-[#0B0B0F] text-dim font-mono text-[10px]">
                    <span className="text-white">{ds.samples}</span>
                  </span>
                  <span className="px-2 py-1 rounded border border-white/5 bg-[#0B0B0F] text-dim font-mono text-[10px]">
                    ~<span className="text-white">{ds.size_mb} MB</span>
                  </span>
                </div>
          ...
          <div
            ...
            className={`p-12 border-2 border-dashed rounded-3xl flex flex-col items-center justify-center text-center cursor-pointer transition-all duration-300 animate-fade-in-up ${
              isDragActive
                ...
                : "border-white/20 bg-[#12121A] hover:border-white/40 hover:bg-[#1A1A28]"
            }`}
          >
            <input {...getInputProps()} />
            <div
              className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 transition-colors ${isDragActive ? "bg-cyan/20 text-cyan" : "bg-white/5 text-dim"}`}
            >
              <Upload size={32} />
            </div>
            <h3 className="text-xl font-bold text-white mb-2">
              {isDragActive ? "Drop files here..." : "Drag & drop your dataset"}
            </h3>
            <p className="text-dim mb-6">or click to browse files</p>
            <div className="flex gap-2">
              <span className="px-3 py-1 rounded-lg bg-cyan/10 text-cyan border border-cyan/20 font-mono text-xs">
                CSV
              </span>
              <span className="px-3 py-1 rounded-lg bg-violet/10 text-violet border border-violet/20 font-mono text-xs">
                PNG
              </span>
              <span className="px-3 py-1 rounded-lg bg-violet/10 text-violet border border-violet/20 font-mono text-xs">
                JPG
              </span>
              <span className="px-3 py-1 rounded-lg bg-blue-500/10 text-blue-400 border border-blue-500/20 font-mono text-xs">
                ZIP
              </span>
            </div>
          </div>
            ...
            <div className="glass-panel p-6 rounded-2xl border border-white/10 animate-fade-in-up">
              <div className="flex items-center justify-between mb-4 border-b border-white/5 pb-4">
                <h4 className="flex items-center gap-2 text-white font-bold">
                  <HardDrive size={18} className="text-cyan" /> Files to Upload
              ...
              <div className="space-y-2">
                {files.map((f) => (
                  <div
                    key={f.id}
                    className="flex items-center p-3 rounded-xl bg-[#12121A] border border-white/5"
                  >
                    <div className="w-10 h-10 rounded-lg bg-white/5 text-dim flex items-center justify-center shrink-0 mr-4">
                      <File size={18} />
                    </div>
          ...
          {loadingDatasets ? (
            ...
            existingDatasets.length > 0 && (
              <div className="glass-panel p-6 rounded-2xl border border-white/10 animate-fade-in-up">
                <div className="flex items-center justify-between mb-4 border-b border-white/5 pb-4">
                  <h4 className="flex items-center gap-2 text-white font-bold">
                    <HardDrive size={18} className="text-violet" /> Uploaded
                    Datasets ({existingDatasets.length})
                  </h4>
                </div>
                <div className="space-y-2">
                  {existingDatasets.map((d) => (
                    <div
                      key={d.id}
                      className="flex items-center p-3 rounded-xl bg-[#12121A] border border-white/5 hover:border-white/10 transition-colors group"
                    >
                      <div className="w-10 h-10 rounded-lg bg-white/5 text-dim flex items-center justify-center shrink-0 mr-4">
                        <File size={18} />
                      </div>
                      <div className="flex-1 min-w-0 mr-4">
                        ...
                        <div className="text-xs text-dim font-mono">
                          {formatSize(d.file_size)} · {d.file_type} ·{" "}
