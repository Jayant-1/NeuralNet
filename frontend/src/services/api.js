...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response?.status === 401) {
      ...
      // Immediate redirect so stale/expired sessions recover predictably.
      if (
...
export const datasetsApi = {
  ...
  listBuiltin: () => api.get("/datasets/builtin"),
  loadBuiltin: (datasetKey, projectId) =>
    api.post("/datasets/builtin/load", {
      dataset_key: datasetKey,
      project_id: projectId,
    }),
  getActive: (projectId) => api.get(`/datasets/${projectId}/active`),
};
